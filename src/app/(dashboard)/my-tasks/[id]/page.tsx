...
> = {
  ...
  request_decision: {
    label: "طلب قرار من المدير",
    status: "بانتظار قرار الإدارة",
    result: "تم طلب قرار من المدير",
  },
  pause: {
    label: "تعليق المهمة",
    status: "معلقة",
  ...
  ready_to_close: {
    label: "جاهزة للإنهاء",
    status: "بانتظار قرار الإدارة",
    result: "المهمة جاهزة للإنهاء بانتظار اعتماد المدير",
  },
};
...
export default function MyTaskDetailPage() {
  ...
  useAutoRefresh(() => load().catch(() => undefined), {
    enabled: authStatus === "authenticated" && session?.user?.role === "employee",
  });

  const closed = task?.status === "مكتملة" || task?.status === "ملغاة";

  async function postUpdate(opts?: { status?: string; result?: string }) {
    if (!task) return;
  ...
  async function onAction(action: EmployeeAction) {
  ...
  const availableActions: EmployeeAction[] = closed
    ? []
    : task.status === "بانتظار قرار الإدارة"
      ? ["start", "pause"]
      : task.status === "لم تبدأ"
        ? ["start", "wait_supplier", "request_decision", "pause"]
        : ["wait_supplier", "request_decision", "pause", "ready_to_close"];

  return (
    <div>
      <PageHeader
        title={`${task.taskNo} — ${task.name}`}
        subtitle="مهمة من مديرك — أضف تحديثك ونفّذ المطلوب"
        actions={
          <Link href="/my-tasks" className="btn btn-secondary">
            العودة لمهامي
          </Link>
        }
      />
      ...
      <div className="mx-auto grid max-w-3xl gap-4">
        <article className="card p-6">
          <div className="mb-4 flex flex-wrap items-start justify-between gap-3">
            <div>
              ...
              <p className="mt-1 text-sm text-[var(--muted)]">
                من: {task.assignedById?.name || "المدير"}
              </p>
            </div>
          ...
          <div className="space-y-3">
            ...
            <div className="rounded-xl border border-[var(--accent)]/40 bg-[var(--accent-soft)] p-4">
              ...
              <p className="mt-2 text-sm font-medium">
                {task.nextAction || "لا يوجد أمر مسجّل بعد"}
              </p>
            </div>
          </div>
        </article>
        ...
        <article className="card space-y-4 p-6">
          <div>
            <h3 className="text-lg font-semibold">تحديث الموظف</h3>
            <p className="mt-1 text-sm text-[var(--muted)]">
              اكتب ما تم إنجازه، ثم احفظ التحديث أو اختر الإجراء
            </p>
          </div>
          ...
          <form onSubmit={onSaveUpdate} className="space-y-3">
            <div className="field">
              ...
              <textarea
                ...
                placeholder="مثال: تم التواصل مع المورد وجمع العرض..."
        ...
        <article className="card p-6">
          ...
          <p className="mb-3 text-xs text-[var(--muted)]">
            الأحدث أولاً — تحديثاتك وأوامر/قرارات المدير
          </p>
