...
export default function HrTaskDetailPage() {
  ...
  > = {
    ...
    request_decision: {
      label: `طلب قرار من ${assignerLabel}`,
      status: "بانتظار قرار الإدارة",
      result: `تم طلب قرار من ${assignerLabel}`,
    },
    pause: {
      label: "تعليق المهمة",
      status: "معلقة",
    ...
    ready_to_close: {
      label: "جاهزة للإنهاء",
      status: "بانتظار قرار الإدارة",
      result: `المهمة جاهزة للإنهاء بانتظار اعتماد ${assignerLabel}`,
    },
  };
  ...
  const load = useCallback(async () => {
    const [t, u] = await Promise.all([
      ...
      apiGet<TimelineItem[]>(`/api/updates?taskId=${params.id}`).catch(
        () => []
      ),
    ]);
    setTask(t);
  ...
  useAutoRefresh(() => load().catch(() => undefined), {
    enabled: authStatus === "authenticated" && session?.user?.role === "hr",
  });
  ...
  async function onAction(action: HrAction) {
  ...
  const availableActions: HrAction[] = closed
    ? []
    : task.status === "بانتظار قرار الإدارة"
      ? ["start", "pause"]
      : task.status === "لم تبدأ"
        ? ["start", "request_decision", "pause"]
        : ["request_decision", "pause", "ready_to_close"];

  const orderFrom =
    assignerRole && assignerRole in ROLE_LABELS
      ? ROLE_LABELS[assignerRole as keyof typeof ROLE_LABELS]
      : "الإدارة";
  ...
  return (
    <div>
      <PageHeader
        ...
        subtitle={`مهمة مُسندة من ${assignerLabel} — أضف تحديثك ونفّذ المطلوب`}
        actions={
          <Link href="/hr/tasks" className="btn btn-secondary">
            العودة لمهامي
          </Link>
        }
      />
      ...
      <div className="mx-auto grid max-w-3xl gap-4">
          ...
          <div className="mb-4 flex flex-wrap items-start justify-between gap-3">
            <div>
              ...
              <p className="mt-1 text-sm text-[var(--muted)]">
                من: {task.assignedById?.name || assignerLabel}
        ...
        <article className="card space-y-4 p-6">
          <div>
            <h3 className="text-lg font-semibold">تحديث الموارد البشرية</h3>
            <p className="mt-1 text-sm text-[var(--muted)]">
          ...
          <form onSubmit={onSaveUpdate} className="space-y-3">
            <div className="field">
              ...
              <textarea
                ...
                placeholder="مثال: تم إعداد قائمة المرشحين ومراجعة العقود..."
        ...
        <article className="card p-6">
          ...
          <p className="mb-3 text-xs text-[var(--muted)]">
            الأحدث أولاً — يشمل تحديثاتك وأوامر/قرارات الإدارة
          </p>
